...
Error Runner<T>::load() {
  ...
  std::string token_generator_method_name, prompt_processor_method_name, blender_method_name;
  ...
  if (eval_mode_ == EvalMode::kBlend) {
    blender_prompt_processor_ = std::make_unique<BlenderPromptProcessor<T>>(
        ...
        kv_manager_.get(),
        blender_method_name,
        typename BlenderPromptProcessor<T>::Metadata{
            context_len_,
            num_heads,
            num_layers,
            prompt_processor_ar_len,
  ...
  if (eval_mode_ == EvalMode::kBlend) {
    blender_prompt_processor_->init_io(
        buffer_manager_.get(), module_->method_meta(blender_method_name));
  }
  return Error::Ok;
}
