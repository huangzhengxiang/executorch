...
class InsertFloatPath(ExportPass):
    """
    ...
    def __init__(self, edge_program: torch.export.ExportedProgram):
        super(InsertFloatPath, self).__init__()
        ...
        with graph_module.graph.inserting_after(node):
            ...
            for user in users:
                if (user.meta.get(QCOM_QUANT_ATTRS) is None 
                    ...
                    user.replace_input_with(node, inserted_node)
                    print("inserted dq node: ", inserted_node.name, inserted_node.target)
    ...
    def _insert(self, graph_module: torch.fx.GraphModule) -> torch.fx.GraphModule:
        for n in graph_module.graph.nodes:
            ...
            # insert dq before fp32 op
            users = list(n.users.keys())
            if (n.meta.get(QCOM_QUANT_ATTRS) \
                and n.meta[QCOM_QUANT_ATTRS][QCOM_ENCODING] in self.q_dq_map \
                and any(
                ...
                print("float path: ", n.name, n.target, [(user.name, user.op, user.target) for user in users])
